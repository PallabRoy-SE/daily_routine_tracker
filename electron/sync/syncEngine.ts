...
import { getDatabasePath, closeDB, initDB } from '../database';
...
async function findBackupFile(accessToken: string): Promise<DriveFileMetadata | null> {
  ...
  return files && files.length > 0 ? (files[0] as DriveFileMetadata) : null;
}

// Create new file metadata on Google Drive
async function createBackupFileMetadata(accessToken: string): Promise<string> {
...
async function uploadDbBinary(accessToken: string, fileId: string, filePath: string): Promise<void> {
...
async function downloadDbBinary(accessToken: string, fileId: string, filePath: string): Promise<void> {
...
function runExecute(dbInstance: any, query: string, params: any[] = []): void {
...
function runSelect(dbInstance: any, query: string, params: any[] = []): any[] {
  const stmt = dbInstance.prepare(query);
  const bindObject: Record<string, any> = {};
  params.forEach((val, index) => {
    const key = `${index + 1}`;
