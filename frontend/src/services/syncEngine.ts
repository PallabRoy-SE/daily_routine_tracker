...
/**
 * Retrieves the stored Google Drive OAuth access token or prompts the user if not set.
 */
export function getAccessToken(): string {
  let token = localStorage.getItem('gdrive_access_token');
  if (!token) {
    token = prompt('Please enter your Google Drive API OAuth Access Token:');
    if (token) {
...
export async function syncWithGDrive(): Promise<void> {
  ...
  if (searchResponse.status === 401) {
    ...
    throw new Error('Google OAuth token is invalid or expired. The token has been reset. Please try again to enter a new token.');
  }
